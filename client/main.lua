...
RegisterNetEvent("ABC_pausemenu:statusUpdated")
AddEventHandler("ABC_pausemenu:statusUpdated", function(status)
...
Citizen.CreateThread(function()
    while true do
        ...
        ScaleformMovieMethodAddParamTextureNameString(makeFontText("~r~Name of the server</FONT>"))
        ScaleformMovieMethodAddParamBool(true)
...
function makeFontText(text)
    return ("<FONT FACE='AMSANSL'> " .. text .. " </FONT>") -----FONT of the menu
end
...
Citizen.CreateThread(function()
    ...
    -- Game
    AddFontTextEntry("PM_PANE_LEAVE", "ODPOJIT SE ZE SERVER")
    ...
    AddFontTextEntry("OCEANA", "Pacifikl ocean")

    -- Blips
    AddFontTextEntry("BLIP_PROPCAT", "Job")
end)
